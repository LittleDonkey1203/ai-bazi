...
import { BaziCompactGrid } from './components/BaziCompactGrid';
import { 
  generateBaZiChart, 
  formatBaZiChart, 
  getGanZhiWuxing,
  ...
  const renderDashboardContent = () => {
    ...
    const renderMatrix = (
      ...
      <div className={wrapperClass}>
        ...
        {/* 桌面端:原 table */}
        <div
          data-testid="bazi-matrix-desktop"
          className={`hidden md:block ${compact ? 'mt-4 overflow-x-auto' : 'mt-5 overflow-x-auto'}`}
        >
          ...
          {chartData && !isGenerating && hasPerformedDivination && (
            ...
            >
              <div className="mx-auto w-full max-w-[1280px] space-y-5 md:space-y-6">
                <motion.div
                  className="overflow-hidden rounded-[24px] md:rounded-[36px] border border-[#242424] bg-[radial-gradient(circle_at_top_left,_rgba(245,236,215,0.18),_rgba(18,18,18,0.96)_40%)]"
                  initial={{ opacity: 0, scale: 0.98 }}
                  animate={{ opacity: 1, scale: 1 }}
                  transition={{ delay: 0.08 }}
                >
                  <div className="grid gap-4 md:gap-5 px-4 py-5 md:px-6 md:py-7 lg:grid-cols-[1.3fr_0.7fr] lg:px-8">
                    <div>
                      <p className="text-[10px] md:text-xs uppercase tracking-[0.28em] md:tracking-[0.36em] text-[#8d887c]">Cantian Style Dashboard</p>
                      <h2 className="mt-2 md:mt-3 text-2xl sm:text-3xl md:text-4xl font-semibold text-white">{chartData.name || '命盘用户'} 的八字仪表盘</h2>
                      <p className="mt-2 md:mt-3 max-w-2xl text-xs md:text-sm leading-6 md:leading-7 text-[#bdb6a8] hidden md:block">
                      ...
                      <div className="mt-3 md:mt-5 flex flex-wrap gap-2">
                        {[chartData.baziText, `${chartData.dayMaster}日主`, `属${chartData.zodiacAnimal}`, `${chartData.decadeFortune.起运年龄}岁起运`].map((item) => (
                          <span key={item} className="rounded-full border border-[#3a362d] bg-[#181614] px-3 md:px-4 py-1.5 md:py-2 text-[11px] md:text-xs text-[#e4dccd]">
                            {item}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="grid gap-2.5 md:gap-3 grid-cols-2 lg:grid-cols-1">
                      {[
                        { label: '阳历', value: chartData.solarDateText },
                        { label: '农历', value: chartData.lunarDateText },
                        { label: '当前大运', value: currentFortune?.干支 || '待识别' },

...
type ExpectedCase = {
  ...
  expectedYongElements: FiveElement[];
  expectedJiElements: FiveElement[];
  expectedClimateApplied: boolean;
  expectedClimateAdjustmentType: ClimateAdjustmentType | null;
};
...
function assertElementList(actual: FiveElement[], expected: FiveElement[], label: string) {
...
function main() {
  ...
  TEST_CASES.forEach((testCase) => {
    ...
    if (mapping.patternType !== testCase.expectedPatternType) {
      throw new Error(
        `${testCase.name} 格局不匹配: expected=${testCase.expectedPatternType} actual=${mapping.patternType}`,
      );
    }
    ...
    if (mapping.climateAdjustment.applied !== testCase.expectedClimateApplied) {
      throw new Error(
        `${testCase.name} 调候是否生效不匹配: expected=${testCase.expectedClimateApplied} actual=${mapping.climateAdjustment.applied}`,
      );
    }

    if (mapping.climateAdjustment.adjustmentType !== testCase.expectedClimateAdjustmentType) {
      throw new Error(
        `${testCase.name} 调候类型不匹配: expected=${testCase.expectedClimateAdjustmentType} actual=${mapping.climateAdjustment.adjustmentType}`,
      );
    }

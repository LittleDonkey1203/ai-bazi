...
type ExpectedCase = {
  ...
  expectedMonthElement: FiveElement;
  expectedStates: Record<FiveElement, SeasonalState>;
};
...
function assertStates(
  actual: Record<FiveElement, SeasonalState>,
  expected: Record<FiveElement, SeasonalState>,
  label: string,
) {
  (Object.keys(expected) as FiveElement[]).forEach((element) => {
    if (actual[element] !== expected[element]) {
...
function main() {
  ...
  TEST_CASES.forEach((testCase) => {
    ...
    const actualStates = result.seasonalAdjustment.adjustments.reduce<Record<FiveElement, SeasonalState>>(
      (accumulator, item) => {
    ...
    if (result.seasonalAdjustment.monthBranchDominantElement !== testCase.expectedMonthElement) {
      throw new Error(
        `${testCase.name} 月支主气不匹配: expected=${testCase.expectedMonthElement} actual=${result.seasonalAdjustment.monthBranchDominantElement}`,
      );
    }

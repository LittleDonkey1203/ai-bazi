import {
  ...
  type BaziInput,
  type BranchMutationApplication,
  type FiveElement,
...
type ExpectedCase = {
  ...
  expectedStemCombinationCount: number;
  expectedBranchMutationLabels: string[];
};
...
function assertMutationLabels(actual: BranchMutationApplication[], expectedLabels: string[], label: string) {
...
function main() {
  ...
  TEST_CASES.forEach((testCase) => {
    ...
    if (result.stemCombinations.applications.length !== testCase.expectedStemCombinationCount) {
      throw new Error(
        `${testCase.name} 天干五合条数不匹配: expected=${testCase.expectedStemCombinationCount} actual=${result.stemCombinations.applications.length}`,
      );
    }

...
type ExpectedCase = {
  ...
  expectedEffectiveDayMasterElement: FiveElement;
  expectedTongDangElements: readonly [FiveElement, FiveElement];
  expectedTongDangScore: number;
  expectedYiDangScore: number;
  expectedTongDangRatio: number;
  expectedYiDangRatio: number;
  expectedTongDangPercent: number;
  expectedYiDangPercent: number;
  expectedMaxSelfElement: FiveElement;
  expectedMaxSelfPercent: number;
  expectedMaxOpposingElement: FiveElement;
  expectedMaxOpposingPercent: number;
  expectedTransformationPassed: boolean;
};
...
function main() {
  ...
  TEST_CASES.forEach((testCase) => {
    ...
    if (pattern.patternType !== testCase.expectedPatternType) {
      throw new Error(
        `${testCase.name} 格局不匹配: expected=${testCase.expectedPatternType} actual=${pattern.patternType}`,
      );
    }

    if (pattern.effectiveDayMasterElement !== testCase.expectedEffectiveDayMasterElement) {
      throw new Error(
        `${testCase.name} 有效日主不匹配: expected=${testCase.expectedEffectiveDayMasterElement} actual=${pattern.effectiveDayMasterElement}`,
      );
    }

    if (
      pattern.tongDangElements[0] !== testCase.expectedTongDangElements[0] ||
      pattern.tongDangElements[1] !== testCase.expectedTongDangElements[1]
    ) {
      throw new Error(
        `${testCase.name} 同党元素不匹配: expected=${testCase.expectedTongDangElements.join(',')} actual=${pattern.tongDangElements.join(',')}`,
      );
    }

    if (normalizeValue(pattern.tongDangScore) !== testCase.expectedTongDangScore) {
      throw new Error(
        `${testCase.name} 同党分数不匹配: expected=${testCase.expectedTongDangScore} actual=${pattern.tongDangScore}`,
      );
    }

    if (normalizeValue(pattern.yiDangScore) !== testCase.expectedYiDangScore) {
      throw new Error(
        `${testCase.name} 异党分数不匹配: expected=${testCase.expectedYiDangScore} actual=${pattern.yiDangScore}`,
      );
    }

    if (normalizeValue(pattern.tongDangRatio) !== testCase.expectedTongDangRatio) {
      throw new Error(
        `${testCase.name} 同党占比不匹配: expected=${testCase.expectedTongDangRatio} actual=${pattern.tongDangRatio}`,
      );
    }

    if (normalizeValue(pattern.yiDangRatio) !== testCase.expectedYiDangRatio) {
      throw new Error(
        `${testCase.name} 异党占比不匹配: expected=${testCase.expectedYiDangRatio} actual=${pattern.yiDangRatio}`,
      );
    }

    if (normalizeValue(pattern.tongDangPercent) !== testCase.expectedTongDangPercent) {
      throw new Error(
        `${testCase.name} 同党百分比不匹配: expected=${testCase.expectedTongDangPercent} actual=${pattern.tongDangPercent}`,
      );
    }

    if (normalizeValue(pattern.yiDangPercent) !== testCase.expectedYiDangPercent) {
      throw new Error(
        `${testCase.name} 异党百分比不匹配: expected=${testCase.expectedYiDangPercent} actual=${pattern.yiDangPercent}`,
      );
    }

    if (pattern.maxSelfElement !== testCase.expectedMaxSelfElement) {
      throw new Error(
        `${testCase.name} 同党最高单元素不匹配: expected=${testCase.expectedMaxSelfElement} actual=${pattern.maxSelfElement}`,
      );
    }

    if (normalizeValue(pattern.maxSelfPercent) !== testCase.expectedMaxSelfPercent) {
      throw new Error(
        `${testCase.name} 同党最高单元素百分比不匹配: expected=${testCase.expectedMaxSelfPercent} actual=${pattern.maxSelfPercent}`,
      );
    }

    if (pattern.maxOpposingElement !== testCase.expectedMaxOpposingElement) {
      throw new Error(
        `${testCase.name} 异党最高单元素不匹配: expected=${testCase.expectedMaxOpposingElement} actual=${pattern.maxOpposingElement}`,
      );
    }

    if (normalizeValue(pattern.maxOpposingPercent) !== testCase.expectedMaxOpposingPercent) {
      throw new Error(
        `${testCase.name} 异党最高单元素百分比不匹配: expected=${testCase.expectedMaxOpposingPercent} actual=${pattern.maxOpposingPercent}`,
      );
    }

    if (pattern.transformation.passed !== testCase.expectedTransformationPassed) {
      throw new Error(
        `${testCase.name} 化气格判定不匹配: expected=${testCase.expectedTransformationPassed} actual=${pattern.transformation.passed}`,
      );
    }

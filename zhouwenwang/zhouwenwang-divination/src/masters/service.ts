...
function extractGeminiTextFromResponse(data: any): string {
  const candidates = data?.candidates;
  if (!Array.isArray(candidates) || candidates.length === 0) {
    throw new Error('AI 未返回有效候选结果');
  }
...
async function simulateStreamingText(
  text: string,
  onUpdate?: (text: string) => void
): Promise<void> {
  if (!onUpdate || !text) {
  ...
  for (let index = 0; index < characters.length; index += 4) {
    current += characters.slice(index, index + 4).join('');
    onUpdate(current);
...
async function requestServerConversationAnalysis(
  serverUrl: string,
  ...
  if (!response.ok) {
    throw new Error(`服务端会话分析失败: HTTP ${response.status}`);
  }
...
async function requestDirectConversationAnalysis(
  apiKey: string,
  contents: GeminiConversationContent[],
): Promise<string> {
  const apiUrl = buildGeminiApiUrl(GEMINI_CONFIG.MODELS.PRIMARY, apiKey);
  ...
  if (!response.ok) {
    throw new Error(`Gemini 会话分析失败: HTTP ${response.status}`);
  }
...
export async function getAIConversationAnalysisStream(
  contents: GeminiConversationContent[],
  onUpdate?: (text: string) => void
): Promise<string> {
  try {
    ...
    if (serverUrl && serverUrl.trim()) {
      try {
        const isServerHealthy = await checkServerHealth(serverUrl);
        if (isServerHealthy) {
          const responseText = await requestServerConversationAnalysis(serverUrl, contents);
          await simulateStreamingText(responseText, onUpdate);
          return responseText;
        }
      } catch (serverError) {
        console.warn('会话分析服务端调用失败，降级到直连 Gemini:', serverError);
      }
    }
    ...
    if (!hasValidApiKey(apiKey)) {
      if (serverUrl && serverUrl.trim()) {
        throw new Error('后端服务不可用，且未配置有效的 Gemini API 密钥');
      }

      throw new Error('请先在设置中配置有效的 Gemini API 密钥');
    }

    const responseText = await requestDirectConversationAnalysis(effectiveApiKey, contents);
    await simulateStreamingText(responseText, onUpdate);
    return responseText;
  } catch (error) {

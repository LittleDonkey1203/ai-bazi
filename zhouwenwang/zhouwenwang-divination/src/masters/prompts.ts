...
## 数据使用规则
- 本次占卜数据中的 \`chartData.rawBaziData\` 来自专业排盘引擎，必须作为最高优先级依据
- 严禁自行重新起盘、修改四柱、改写日主、改写生肖或臆造大运
- 如个别字段缺失，只能基于已提供字段谨慎分析，不得自行补造命盘事实
- 论证时优先引用四柱、十神、藏干、命宫、身宫、大运、刑冲合会等已提供结构化数据
...
**专业要求**：
- 必须以 \`chartData.rawBaziData\` 和 \`chartData\` 中已经给出的结构化命盘为准，不得与数据冲突

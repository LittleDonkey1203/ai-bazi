...
export class SeasonalMultiplierApplier {
  public applySeasonalMultipliers(
    ...
    const adjustments = ELEMENT_KEYS.map((element) =>
      this.adjustSingleElement(element, monthBranchDominantElement, inputScores, adjustedScores),
    );
  ...
  private getMultiplierByState(state: SeasonalState): number {
    switch (state) {
      case '旺':
        return 1.5;
      case '相':

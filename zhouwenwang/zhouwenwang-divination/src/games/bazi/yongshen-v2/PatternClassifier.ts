import { getDestroyerElement, getGeneratorElement, STEMS } from './constants';
import type {
  BaseScoreResult,
  FiveElement,
...
export class PatternClassifier {
  public classifyPattern(
    ...
    const originalDayMasterElement = STEMS[baseScore.baziInput[4] as Stem];
    const transformation = this.evaluateTransformation(
      baseScore,
      stemCombinations,
      monthBranchDominantElement,
    );
    const effectiveDayMasterElement = transformation.passed
      ? (transformation.targetElement as FiveElement)
      : originalDayMasterElement;
    const tongDangElements = [
      effectiveDayMasterElement,
      getGeneratorElement(effectiveDayMasterElement),
    ] as const;
    const tongDangScore = roundValue(
      (inputScores[tongDangElements[0]] || 0) + (inputScores[tongDangElements[1]] || 0),
    );
    ...
    const maxSelfPercent =
      totalScore === 0 || !maxSelfElement ? 0 : roundValue((inputScores[maxSelfElement] / totalScore) * 100);
    ...
    const maxOpposingPercent =
      totalScore === 0 || !maxOpposingElement
        ? 0
  ...
  private evaluateTransformation(
    ...
    const dayRelatedCombination =
      stemCombinations.applications.find((application) => application.pairIndexes.includes(4)) || null;
  ...
  private findBlockingStems(
    baseScore: BaseScoreResult,
    destroyerElement: FiveElement,
  ): TransformationBlockingStem[] {
    const stemIndexes: StemScoreIndex[] = [0, 2, 4, 6];
    return stemIndexes.reduce<TransformationBlockingStem[]>((result, index) => {
      const stem = baseScore.baziInput[index] as Stem;
  ...
  private getOpposingElements(
    tongDangElements: readonly [FiveElement, FiveElement],
  ): FiveElement[] {
    return (['金', '木', '水', '火', '土'] as FiveElement[]).filter(
      (element) => !tongDangElements.includes(element),
    );
  }

  private findMaxElementInGroup(
    ...
    return elements.reduce<FiveElement>((currentMax, candidate) => {
      return scores[candidate] > scores[currentMax] ? candidate : currentMax;
    }, elements[0]);
  }

  private resolveRegularPatternType(

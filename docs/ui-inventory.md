...
|----|------|------|----|----|---------|---------|-------|--------|------|
| S001 | zhouwenwang/zhouwenwang-divination-mobile/index.html | html | shared | - | viewport meta / Tailwind CDN config / brand colors | 否 | 0 | TODO | 工程根入口;内嵌 Tailwind config(brand-orange 等),后续主色迁移时需在此声明扩展色 |
| S002 | zhouwenwang/zhouwenwang-divination-mobile/src/main.tsx | component | shared | - | createRoot / StrictMode | 否 | 0 | TODO | React 入口,仅挂载 App,无 UI 元素;batch 0 时若不需注入全局 Provider/字体引导,改 SKIP |
| S003 | zhouwenwang/zhouwenwang-divination-mobile/src/App.tsx | layout | shared | - | Router / Layout / ElectronInfo | 否 | 0 | TODO | 顶层路由壳,依据 `isElectron()` 切换 BrowserRouter / HashRouter;batch 0 时若不需注入全局 Provider/字体引导,改 SKIP |
| S004 | zhouwenwang/zhouwenwang-divination-mobile/src/index.css | style | shared | - | reset / dark theme / 动画 keyframes(fadeIn/slideIn 等) | 否 | 0 | TODO | 含 `@media (prefers-color-scheme)`(已注释禁用,强制暗色)— 不算视口断点;design-tokens 的 CSS 变量后续应放此处 |
| S005 | zhouwenwang/zhouwenwang-divination-mobile/src/App.css | style | shared | - | logo / card / read-the-docs 残留 | 否 | 0 | TODO | 含 `@media (prefers-reduced-motion)` — 偏好查询,不算视口断点;模板残留样式可能可清理但需确认 |
| S006 | zhouwenwang/zhouwenwang-divination-mobile/src/hooks/useBreakpoint.ts | hook | shared | - | matchMedia(MOBILE/DESKTOP) → `{ isMobile, isTablet, isDesktop }` | 是 | 0 | TODO | partial - 已完成响应式骨架(本次主色/语义化迁移可能无需改动);改造期所有断点判断的唯一来源 |
| S007 | zhouwenwang/zhouwenwang-divination-mobile/src/components/layout/Layout.tsx | layout | shared | - | MobileDetector / Marquee / Sidebar / BottomNav / MainContent 装配 | 是 | 0 | TODO | partial - 已完成响应式骨架;`isMobile` 控制 Sidebar/BottomNav 切换,主内容 `paddingBottom: 5rem` |
| S008 | zhouwenwang/zhouwenwang-divination-mobile/src/components/layout/Sidebar.tsx | layout | shared | - | nav-list / collapse-btn / settings-entry / 大师 badge | 否 | 0 | TODO | partial - 已完成响应式骨架(由 Layout 控制是否渲染),其内部样式仍是桌面定宽,可能需要在折叠态下做更细的视觉调整 |
| S009 | zhouwenwang/zhouwenwang-divination-mobile/src/components/layout/BottomNav.tsx | layout | shared | - | tab-bar / more-sheet(底部抽屉) / safe-area-inset | 是 | 0 | TODO | partial - 已完成响应式骨架;`md:hidden` 双重保险,内置"更多"抽屉承载第 5 项以后的导航 |
| S010 | zhouwenwang/zhouwenwang-divination-mobile/src/components/layout/MainContent.tsx | layout | shared | - | Routes / Suspense / framer-motion 页面切换 | 是 | 0 | TODO | partial - 已完成响应式骨架;`px-4 sm:px-6 lg:px-8 xl:px-12` 容器内边距已分级 |
| F001 | zhouwenwang/zhouwenwang-divination-mobile/src/pages/HomePage.tsx | page | web | / | hero / 游戏卡片 grid / 大师介绍 / cta | 是 | 1 | TODO | 仅排版断点(`md:text-6xl`);布局 grid 未分级,卡片在窄屏可能堆叠不佳 |
| F002 | zhouwenwang/zhouwenwang-divination-mobile/src/components/MasterSelectorDemo.tsx | page | web | /masters | header / 大师卡片 / 调试信息面板 | 否 | 1 | TODO | 注:CLAUDE.md 称其为"大师选择演示页",定位为页;包裹 MasterSelector |
...
| F013 | zhouwenwang/zhouwenwang-divination-mobile/src/masters/MasterSelector.tsx | component | web | - | 大师卡片网格 / loading / error 态 | 否 | 1 | TODO | 复用组件:HomePage、MasterSelectorDemo、SettingsModal 都用它;批次建议跟随 MasterSelectorDemo |

...
| F003 | zhouwenwang/zhouwenwang-divination-mobile/src/games/liuyao/LiuYaoPage.tsx | page | web | /liuyao | 问题输入 / 起卦动画(video) / 卦象展示 / AI 流式分析 | 是 | 2 | DONE | 仅排版断点(`md:text-5xl`);卦象六爻矩阵需移动端竖向适配 | batch 2 完成 / 横向溢出 409→375 = 34px 消除 / D5a 卦名爻名 font-serif + D5b 标题渐变 paper→bronze→vermilion + D6 主 CTA 绛红 + D7 卦名墨青 + M17 异常 margin 修正 |
| F004 | zhouwenwang/zhouwenwang-divination-mobile/src/games/qimen/QiMenPage.tsx | page | web | /qimen | 时间选择器 / 九宫格盘 / quick-questions / AI 流式分析 | 是 | 2 | DONE | 仅排版断点;九宫格在 < 360 宽屏需收紧字号或允许横滚 | batch 2 完成 / 横向溢出 409→375 = 34px 消除 / D2 收紧字号 + 兜底 B 横滚 + D8 边框中性深灰 + D9 信息条 mobile 2×2 + DA 地盘天干 mobile 显示修复 + DD datetime 抖动修复 + 同 LiuYao D5a/D5b/D6 落地 |
| F005 | zhouwenwang/zhouwenwang-divination-mobile/src/games/bazi/BaZiPage.tsx | page | web | /bazi | 出生信息 form / 多视图 tabs / 命盘 grid / 神煞 / 大运 / 流年 / AI 对话 / 案例存档 | 是 | 3 | DONE | **本工程最复杂页**(2800+ 行);已大量使用 `md:` `lg:` `xl:` 分级,真正的响应式优等生但仍有 `xl:hidden` 与桌面专属侧栏要再检视 | batch 3 完成(~106 修改块) / D8 dashboard 双栏断点 4 阶段演进(lg→xl→2xl→200px 左栏)Playwright 1024-1920 五视口全免滚 / hero h1+h3 paper→bronze→vermilion 渐变 + font-serif / 主 CTA + 录入 active + dashboard tab active 全 brand 绛红 / 6 tab dark 灰系 token 化 + light 2 tab 米白/暖色调登记 / W1c table min-w 1080→860 / D9 起盘视频 mobile aspect-[16/9] / 详见 audit-batch-3.md |
| F006 | zhouwenwang/zhouwenwang-divination-mobile/src/games/bazi/components/BaziCompactGrid.tsx | component | web | - | 紧凑命盘 grid(主星/天干/地支/藏干/纳音/神煞) | 否 | 3 | DONE | BaZiPage 子组件,固定列宽,移动端横滚展示;批次跟随 BaZiPage | batch 3 完成(31 修改块, 5 轮迭代) / 4 个新可选 prop(hideRows/dataMinWidth/labelWidth/hideColumnSubtitle 向后兼容) / SHORT_GODS_MAP 短神煞映射 + font-serif × 5 + paper-50 token / date sticky cell 内层 leading-4 div 对齐修复 / 详见 audit-batch-3.md |
